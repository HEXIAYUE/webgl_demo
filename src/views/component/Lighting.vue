	...
	methods: {
		...
		init() {
			...
			this.camera = new THREE.PerspectiveCamera(
				75,
				this.$refs.lighting.clientWidth / this.$refs.lighting.clientHeight,
				0.1,
				1000
			);
			...
			//使用div的宽度，使模型一直在div中间 this.$refs.lighting.clientWidth window.innerWidth\
			this.renderer.setSize(
				this.$refs.lighting.clientWidth,
				this.$refs.lighting.clientHeight
			);
			// 颜色
			this.renderer.setClearColor(new THREE.Color(0x333333));
			// 将渲染器挂载在div app下
			const lighting = document.getElementById('lighting');
			lighting.appendChild(this.renderer.domElement);
			// 添加贴图
