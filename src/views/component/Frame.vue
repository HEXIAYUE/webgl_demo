	...
	methods: {
		...
		init() {
			...
			this.camera = new THREE.PerspectiveCamera(
				75,
				this.$refs.Frame.clientWidth / this.$refs.Frame.clientHeight,
				0.1,
				1000
			);
			...
			//使用div的宽度，使模型一直在div中间 this.$refs.Frame.clientWidth window.innerWidth\
			this.renderer.setSize(this.$refs.Frame.clientWidth, this.$refs.Frame.clientHeight);
			// 颜色
			this.renderer.setClearColor(new THREE.Color(0x333333));
			//获得前后两次执行该方法的时间间隔
			this.clock = new THREE.Clock();
			// 将渲染器挂载在div app下
			const Controller = document.getElementById('Frame');
